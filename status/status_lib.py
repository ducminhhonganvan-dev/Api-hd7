...
def get_user_input(message):
 parts = message.split()
 if len(parts) < 2:
  return "[b][c][FFFF00]Vui long nhap ID.\nVi du: {} 12345678".format(
   message.split()[0]
  )
...
def RequestAddingFriend(uid, token, url):
 ...
 headers = {
  "ReleaseVersion": "OB53", "X-GA": "v1 1",
  "Authorization": "Bearer {}".format(token)}
 packet = pb_encode({1: 13333333735, 2: int(uid), 3: 22})
 payload = AES_CBC128(packet, key, iv)
 res = requests.post(url, data=payload, headers=headers)
 return res.content
...
def send_info(uid, token, url):
 ...
 def convert_time(timestamp):
  return datetime.datetime.fromtimestamp(timestamp).strftime("Ngay %d, thang %m, nm %Y, %H gi, %M phut")
